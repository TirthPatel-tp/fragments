...
describe('GET /v1/fragments', () => {
  ...
  test('not logged in requests are denied', () => request(app).get('/v1/fragments').expect(401));
  ...
  test('Get request with ?expand=1 should get expanded fragment metadata', async () => {
    ...
    expect(getRes.body.fragments).toContainEqual(fragment1);
    expect(getRes.body.fragments).toContainEqual(fragment2);
  });
});
